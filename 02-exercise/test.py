...
def parseEdition(line, editors_arr, composition):
    edition_name = re.search("Edition: (.*[\S].*)", line)
    edition_name = edition_name.group(1) if edition_name is not None else None
    return Edition(composition, editors_arr, edition_name)
...
def load(filename):
    ...
    for line in block:
        ...
        edition = parseEdition(line, editors_arr, composition)
