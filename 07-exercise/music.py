...
def get_tone(fundamental_freq, freq):
    pitches_len = len(PITCHES)
    log_distance = pitches_len * (math.log2(freq) - math.log2(fundamental_freq * pow(2, -(pitches_len + 9) / pitches_len)))
    ...
    if octaves_difference >= 0:
        ...
        additional_chars = (',' * (-1 * octaves_difference))
...
def avg_over_channels(frame_bytes, num_channels):
    i = 0

    new_frame_bytes = []
    while i <= (len(frame_bytes) - num_channels):
        averaged_frame_byte = 0
        for channel_idx in range(num_channels):
            averaged_frame_byte += frame_bytes[i + channel_idx]
...
def parse_frames(music_file, frames_per_sec, num_of_sec, num_of_channels):
    ...
    parsed_bytes = avg_over_channels(parsed_bytes, int(num_of_channels))
...
def get_peaks(music_file, given_freq):
    ...
    for _ in range(int(number_of_windows - 1)):
        ...
        if len(top_peaks) > 0:
            ...
            if len(all_peaks) > 0 and all_peaks[-1]['to_time'] == from_window_time and all_peaks[-1][
                'result'] == tones_str:
